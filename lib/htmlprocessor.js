...
//
var getBlocks = function (dest, tmp, dir, content) {
    ...
    lines.forEach(function (l) {
        ...
        if (block && last) {
            ...
            if (asset && asset[2]) {
                ...
                var src = path.join(originDir, asset[2]);

                // custom less identification
                var less = l.match(/rel=['"]stylesheet\/less['"]/);
                if(less){
                    if(!last.less){
                        last.less = [];
                    }

                    src = path.join(tmp, asset[2]);
                    var folder = path.dirname(src), filename = path.basename(src).split('.').shift();
                    src = path.join(folder,filename+'.css');

                    last.less.push({
                        src: path.join(originDir, asset[2]),
                        dest: src
                    });
                }
...
//
var HTMLProcessor = module.exports = function (src, dest, tmp, content, revvedfinder, logcb) {
    // FIXME: Check consistency of the file object
    this.src = src;
    this.dest = dest || src;
    this.tmp = tmp || this.dest;
    this.content = content;
    this.relativeSrc = path.relative(process.cwd(), src);
    this.revvedfinder = revvedfinder;
    this.linefeed = /\r\n/g.test(this.content) ? '\r\n' : '\n';
    this.blocks = getBlocks(this.dest, this.tmp, this.relativeSrc, this.content);
