...
//
ConfigWriter.prototype.process = function (file, config, grunt, encoding) {
  var self = this;
  var lfile = file;
  ...
  if (_.isString(file)) {
    lfile = new File(file, grunt, encoding);
  }
  ...
  lfile.blocks.forEach(function (block) {
    ...
    self.forEachStep(block.type, function (writer, last) {
      ...
      context.outDir = last ? lfile.searchPath[0] : path.join(self.staging, writer.name);
      context.last = last;
